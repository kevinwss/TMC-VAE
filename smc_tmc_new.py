...
def generate_next_TMC(tmc_old):
    ...
    def sample_partition(node_to_split,partition_len):# partition_len<=upper(N/2)
        ...
        '''
        left = node_to_split.z_nodes[:partition_len]
        right = node_to_split.z_nodes[partition_len:]
...
def Gaussian(z,mean,var): #z(batch_size,1)
    ...
    ans = ((np.sqrt(2*pi*var))**-1) *np.exp(-0.5*((z-mean)**2/var)) + 0.00000000000000001
    #print("ans",ans)
    return ans
...
def update_TMC(z,P_l,tau_l_star = None, is_first = 1):# z (batch_size,z_dim)
    ...
    while l<L:
        ...
        for c in range(1,C):  #why?
            ...
            #print("j[c]",j[c])
            #P_l[c][l] = P_l[j[c]][l] #
            P_l[c][l]  = TMC(z_dim) # new tmc
            P_l[c][l].copy(P_l[j[c]][l])
...
for it in range(1,4000):
    ...
    # Use SMC to learn new latent variables
    if it % 400 == 0:
        #P_l,tau_star,tau_l_star = init_variables()
    ...
    if it % 100 == 0:
        ...
        X_train,X_test = z_sample_[:train,:],z_sample_[train:,:]
        ...
        test_acc = acc(predict,Y_test)
        print("acc",test_acc)
        test_accs.append(test_acc)
        print("avg_acc",sum(test_accs)/len(test_accs))
        #---------------------------------    
        print()
